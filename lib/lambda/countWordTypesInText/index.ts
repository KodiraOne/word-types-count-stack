...
export const handler: Handler = async (event) => {
  ...
  const words: string[] = requestBody.text.toLowerCase().split(/\s+/);

  const wordTypesCount: WordTypesCount = {
    "noun": 0,
    "verb": 0,
    "adjective": 0,
    "adverb": 0,
  ...
  words.forEach((word: string) => {
    Object.keys(dictionary).forEach((type: string) => {
      if (dictionary[type].includes(word)) {
        wordTypesCount[type]++;
      }
    });
  });
  ...
  return {
    statusCode: 200,
    body: JSON.stringify(wordTypesCount),
  };
};